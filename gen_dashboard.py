...
table.agt th{font-weight:600;opacity:.65;cursor:pointer;user-select:none;position:sticky;top:0;transition:background .12s,opacity .12s;border-bottom:2px solid transparent}
...
function filtered() {
  return srcData().filter(r => {
    ...
    if (S.tab === 'monthly') {
      if (r.month < S.monthFrom || r.month > S.monthTo) return false;
    } else {
      if (r.week < S.weekFrom || r.week > S.weekTo) return false;
    }
    return true;
  });
}
...
const C_MAP = {
  ...
  problem: { canvas: 'problem-chart-wrap', table: 'problem-tbl-wrap', note: 'problem-note',
             modes: ['overall','country','agent','table'], kpi: 'problem-kpi-wrap' }
};
